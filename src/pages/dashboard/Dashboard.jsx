...
function DashboardOverview() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch current user
      const userResponse = await fetch((import.meta.env.VITE_API_URL || 'http://localhost:5001/api') + '/auth/me', { headers });
      const userData = await userResponse.json();
      setCurrentUser(userData.user);

      // Fetch questions
      const questionsResponse = await fetch((import.meta.env.VITE_API_URL || 'http://localhost:5001/api') + '/teacher/questions', { headers });
      const questionsData = await questionsResponse.json();

      // Fetch exams
      const examsResponse = await fetch((import.meta.env.VITE_API_URL || 'http://localhost:5001/api') + '/teacher/exams', { headers });
      const examsData = await examsResponse.json();

      // Fetch students
      const studentsResponse = await fetch((import.meta.env.VITE_API_URL || 'http://localhost:5001/api') + '/teacher/students', { headers });
